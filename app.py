...
def login():
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('index.html', us=True)
...
def apply():
    if request.method == "POST":
        ...
        db.child("users").child(person["uid"]).update(request.form)

        # Upload resume to storage
        storage.child(f'resumes/{person["fname"]}_{person["sname"]}.pdf').put(request.files['resume'])
